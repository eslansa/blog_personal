  ...
  return (
    <div className=" mb-4 p-4 border border-gray-900 rounded-md shadow-sm shadow-red-950 hover:shadow-md hover:bg-red-500 hover:text-white hover:dark:bg-gray-950">
      <Link href={`/posts/${post?.slug?.current}`}>
        ...
        <p className={`${dateFont.className} my-2 text-red-700`}>{new Date(post?.publishedAt).toDateString()}</p>
      ...
      <div>
        {post?.tags?.map((tag, index) => (
          <span key={tag?._id || index} className='mr-2 p-1 rounded-sm text-sm lowercase dark:bg-gray-950 border dark:border-gray-900 mt-2 hover:text-red-100 dark:border-gray-90 px-2 py-1 shadow-sm max-w-xs mx-auto'>
            {tag?.name}
          </span>
        ))}
      </div>
